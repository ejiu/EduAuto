//修改图片配对成功后原始图片隐藏机制
class WordLinkPanel1 extends PageBase {
    public constructor() {
        super();
        this.skinName = "WordLinkSkin1";
        ...
        this.interactiveCount = this.jsonData['word_link1_info']['cards'].length;
        var tImg:eui.Image;
        for(var i:number = 0; i<this.interactiveCount; ++i)
        {      
            tImg = this.getInteractImg(i);      
            tImg.source = this.jsonData['word_link1_info']['cards'][i];  
    ...
    private resetAllCard()
    {
        ...
        {      
            ...
            tImg.rotation = 0;//
            tImg.source = this.jsonData['word_link1_info']['cards'][this.cardEndL[i]];  
        }
    ...
    private clickCard(curClickCard:number):void
    {
        ...
        if(-1 != this.lastClickNum)
        {
            ...
            if(tLastImg.source.toString().substr(6,2) == tImg.source.toString().substr(6,2))
            {
                ...
                egret.Tween.get(tImg).to({x:posX,y:posY}, 300, egret.Ease.sineOut).call(this.aniCenterCard,this,[tImg,this.rightNum]); 
    ...
    //修改图片配对成功后隐藏机制
    //以train101_png,tran201_png为例，取第6位字符判断奇偶，奇数则隐藏图片，偶数则移动到ResultPos处；
    private aniCenterCard(tAniImg:eui.Image, tNum:number):void
    {
        if(parseInt(tAniImg.source.toString().substr(5,1)) % 2 == 1)
        {
