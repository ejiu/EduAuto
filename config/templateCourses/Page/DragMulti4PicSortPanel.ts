...
class DragMulti4PicSortPanel extends PageBase {
    public constructor() {
        super();
        this.skinName = "DragMulti4PicSortSkin";

        this.jsonData = RES.getRes("page_info_json");
        this.interactiveCount = this.jsonData['drag_multi_4picSort_info']['items'].length;
        ...
        {        
            ...
            tImg.source = this.jsonData['drag_multi_4picSort_info']['items'][i];

            tBgImg = this.getInteractBgImg(i);
            tBgImg.source = this.jsonData['drag_multi_4picSort_info']['items'][i]; 
            tBgImg.x = tImg.x;
            tBgImg.y = tImg.y;
