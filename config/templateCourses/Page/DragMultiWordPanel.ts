...
class DragMultiWordPanel extends PageBase {
    public constructor() {
        super();
        this.skinName = "DragMultiWordSkin";

        this.jsonData = RES.getRes("page_info_json");
        this.interactiveCount = this.jsonData['drag_multi_word_info']['items'].length;
        
        var tLab:eui.Label;
        for(var i:number = 0; i<this.interactiveCount; ++i)
        {      
            tLab = this.getInteractWord(i);  
            tLab.text = this.jsonData['drag_multi_word_info']['items'][i]['word'];
            this.originPosesNumL.push(new egret.Point(tLab.x, tLab.y));
        }
        this.interactiveEndCount = 2;
        this.result_word0.text = this.jsonData['drag_multi_word_info']['result'][0];
        this.result_word1.text = this.jsonData['drag_multi_word_info']['result'][1];

        var tNeedCheck = this.jsonData["drag_multi_word_info"]['needCheck'];
        if(tNeedCheck)
        {           
    ...
    {  
        ...
        {    
            ...
            for(var j:number = 0; j<tCount; ++j)
            {
                ...
                {
                    hasDeal = true;
                    if(this.jsonData['drag_multi_word_info']['items'][this.chooseNumL[j]]['word']
                        == this.jsonData['drag_multi_word_info']['result'][i])
                    {
                        CheckAnswerPanel.checkAnswer().showResult(new egret.Point(tResultLab.x+offsetX,tResultLab.y+offsetY),true);                        
                    }
                    else
                    {
                        CheckAnswerPanel.checkAnswer().showResult(new egret.Point(tResultLab.x+offsetX,tResultLab.y+offsetY),false); 
