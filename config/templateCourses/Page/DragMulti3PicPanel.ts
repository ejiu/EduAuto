...
class DragMulti3PicPanel extends PageBase {
    public constructor() {
        super();
        this.skinName = "DragMulti3PicSkin";

        this.jsonData = RES.getRes("page_info_json");
        this.interactiveCount = this.jsonData['drag_multi_3pic_info']['items'].length;
        ...
        {        
            ...
            tImg.source = this.jsonData['drag_multi_3pic_info']['items'][i];

            tBgImg = this.getInteractBgImg(i);
            tBgImg.source = this.jsonData['drag_multi_3pic_info']['items'][i]; 
            tBgImg.x = tImg.x;
            tBgImg.y = tImg.y;
    ...
    public initUI(): void {
        ...
        //     { text: "house", style: { "textColor": 0xFF0000, "href": "event:event1" } },
        // ];
    ...
    private dealMouseupMsg(tTargetIndex:number, tPosX:number, tPosY:number)
    {
        ...
        {     
            ...
            tEndImage = this.getInteractResultImg(i);
        ...
        if(isHit)
        {
            ...
            this.dealNumL.push(parseInt(tEndImage.name));
